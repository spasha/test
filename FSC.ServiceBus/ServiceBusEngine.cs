...
namespace FSC.ServiceBus
{
  ...
  public class ServiceBusEngine
  {
    ...
    {
      _logger.LogInformation("[FSC.ServiceBus] Starting azure service bus clients");

      if (_options.Value.Settings.Enabled == false)
      {
        _logger.LogInformation("[FSC.ServiceBus] Reception and dispatch of messages have been deactivated through configuration.");
      }
      if (_options.Value.Settings.Enabled && _options.Value.Settings.ReceiveMessages == false)
      {
        _logger.LogInformation("[FSC.ServiceBus] Reception of messages have been deactivated through configuration.");
      }
    ...
    private void BuildReceptions()
    {
      ...
      var duplicatedHandlers = regs.GroupBy(o => new
      {
        o.Options.ClientType,
        o.Options.ResourceId,
        o.HandlerType
      ...
      var duplicateEvenTypeIds = regs.GroupBy(o => new
      {
        o.Options.ClientType,
        o.Options.ResourceId,
        o.PayloadTypeId
    ...
    private async Task RegisterReceiver(ReceiverOptions[] receivers)
    {
      ...
      {
        _logger.LogError(ex, "[FSC.ServiceBus] Initialization of client '{ResourceId}': Failed", receiverOptions.ResourceId);
      }
    }
    ...
    private void RegisterSender(ClientOptions[] senders)
    {
      ...
      _logger.LogInformation("[FSC.ServiceBus] Initialization of sender client '{ResourceId}': Start", options.ResourceId);
      ...
      if (_options.Value.Settings.Enabled == false)
      {
        ...
        _logger.LogInformation(
            "[FSC.ServiceBus] Initialization of sender client '{ResourceId}': Client deactivated through configuration", options.ResourceId);
        return;
      }
      ...
      try
      {
        ...
        _logger.LogInformation("[FSC.ServiceBus] Initialization of sender client '{ResourceId}': Success", options.ResourceId);
      }
      catch (Exception ex)
      {
        _logger.LogError(ex, "[FSC.ServiceBus] Initialization of sender client '{ResourceId}': Failed", senders.First().ResourceId);
    ...
    {
      _logger.LogInformation("[FSC.ServiceBus] Stopping azure service bus clients");
